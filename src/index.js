...
async function main() {
  ...
  const autoInstall =
    options.installDeps || process.env.REMOTE_STDIO_AUTO_INSTALL === "1";
  ...
  if (options.checkPty) {
    const result = await checkNodePty({
      autoInstall,
      repairInstall: options.installDeps,
    });
    console.log(
      result.ok
        ? "node-pty 사용 가능"
        : `node-pty 사용 불가: ${result.error ?? "알 수 없는 오류"}`,
    );
  ...
  if (options.installDeps && !options.interactive && !options.prompt) {
    ...
    console.log(
      result.ok
        ? "node-pty 설치 및 확인 완료"
        : `node-pty 설치 후 확인 실패: ${result.error ?? "알 수 없는 오류"}`,
    );
    process.exit(result.ok ? 0 : 1);
  }
...
async function runInteractive({ options, agents, pty }) {
  ...
  try {
    console.log(
      "인터랙티브 모드를 시작합니다. 종료하려면 /exit를 입력하십시오.",
    );
    for (;;) {
...
async function runSplitInteractive({ options, agents, pty }) {
  ...
  relay?.setStatus?.("에이전트 준비 대기 중", true);
  relay?.setInputLine?.("", true);
    ...
    await new Promise((resolve) => {
      const onKeypress = async (str, key = {}) => {
        ...
        if (key.name === "return" || key.name === "enter") {
          ...
          inputLine = "";
          relay?.setInputLine?.(inputLine, true);
          ...
          if (prompt === "/agents") {
            relay?.setStatus?.(
              `활성 에이전트: ${agents.map((agent) => agent.name).join(", ")}`,
              true,
            );
            return;
          }
          if (prompt === "/help") {
            relay?.setStatus?.("명령: /exit, /agents, /help", true);
            return;
          }

          busy = true;
          relay?.setStatus?.("프롬프트 전송 및 응답 대기 중", true);
          try {
            await runner.sendPrompt(prompt);
            relay?.setStatus?.("준비됨", true);
          } catch (error) {
            relay?.setStatus?.(`오류: ${error.message}`, true);
          } finally {
            busy = false;
          }
          return;
    ...
    if (!options.json) {
      for (const agentResult of result.results) {
        if (agentResult.status !== "success") {
          console.error(
            `[${agentResult.agent}] ${agentResult.status}: ${agentResult.error ?? "오류"}`,
          );
        }
      }
    }
...
async function resolveRuntime({ options, autoInstall }) {
  ...
  try {
    ...
    if (!ptyCheck.ok) {
      console.error(
        `node-pty 사용 불가: ${ptyCheck.error ?? "알 수 없는 오류"}`,
      );
